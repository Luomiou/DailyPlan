...
%              posterior_perch_bri——鲈鱼亮度样本分到鲈鱼类别的后验概率
%        posterior_pervssalmon_bri——鲈鱼亮度样本分到三文鱼类别的后验概率
%          posterior_salmon_length——三文鱼长度样本分到三文鱼类别的后验概率
%      posterior_salvsperch_length——三文鱼长度样本分到鲈鱼类别的后验概率
%             posterior_salmon_bri——三文鱼亮度样本分到三文鱼类别的后验概率
%         posterior_salvsperch_bri——三文鱼亮度样本分到鲈鱼类别的后验概率
...
title('长度 后验概率曲线');
%鲈鱼的亮度
TY.y_perch_bri=C.a_perch_train_bri*gaussmf(perch_test(:,2),[A.sigma_perch_train_bri A.mul_perch_train_bri]);%将鲈鱼的测试样本分给鲈鱼的类条件概率
TY.y_per_vssalmon_bri=C.a_salmon_train_bri*gaussmf(perch_test(:,2),[A.sigma_salmon_train_bri A.mul_salmon_train_bri]);%将鲈鱼的测试样本分给三文鱼的类条件概率
Pos.posterior_perch_bri=TY.y_perch_bri*p1./(TY.y_perch_bri*p1+TY.y_per_vssalmon_bri*p2);%将鲈鱼的测试样本分给鲈鱼的后验概率
Pos.posterior_pervssalmon_bri=TY.y_per_vssalmon_bri*p2./(TY.y_perch_bri*p1+TY.y_per_vssalmon_bri*p2);%将鲈鱼的测试样本分给三文鱼的后验概率
figure(3)
subplot(2,2,3);
...
title('长度 后验概率曲线');
%三文鱼的亮度 
TY.y_salmon_bri=C.a_salmon_train_bri*gaussmf(salmon_test(:,2),[A.sigma_salmon_train_bri A.mul_salmon_train_bri]);%将鲈鱼的测试样本分给鲈鱼的类条件概率
TY.y_sal_vsperch_bri=C.a_perch_train_bri*gaussmf(salmon_test(:,2),[A.sigma_perch_train_bri A.mul_perch_train_bri]);%将鲈鱼的测试样本分给三文鱼的类条件概率
Pos.posterior_salmon_bri=TY.y_salmon_bri*p2./(TY.y_salmon_bri*p2+TY.y_sal_vsperch_bri*p1);%将鲈鱼的测试样本分给鲈鱼的后验概率
Pos.posterior_salvsperch_bri=TY.y_sal_vsperch_bri*p1./(TY.y_salmon_bri*p2+TY.y_sal_vsperch_bri*p1);%将鲈鱼的测试样本分给三文鱼的后验概率
figure(3)
subplot(2,2,4);
plot(salmon_test(:,2),Pos.posterior_salmon_bri,'b.');
hold on
plot(salmon_test(:,2),Pos.posterior_salvsperch_bri,'r*');
legend('p(三文鱼|三文鱼测试样本)','p(鲈鱼|三文鱼测试样本)','Location','East');
xlabel('三文鱼亮度测试样本');
ylabel('后验概率');
title('亮度 后验概率曲线'); 
...
P.pc_bri=sum(Ap_bri(:)>0)/size(Ap_bri,1);%表示以鲈鱼亮度测试样本集作为分类样本，分类的正确率
P.pw_bri=sum(Ap_bri(:)<0)/size(Ap_bri,1);
%三文鱼的5000个亮度测试样本
As_bri=TY.y_salmon_bri-TY.y_sal_vsperch_bri;
S.sc_bri=sum(As_bri(:)>0)/size(As_bri,1);%表示以三文鱼长度测试样本集作为分类样本，分类的正确率
